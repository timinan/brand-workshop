...
/**
 * Thin wrapper that runs all three phases sequentially, auto-picking the
 * Namer's top_pick as the chosen name. Used by the legacy `/api/workshop`
 * route and the preset baking script. Will be removed once those callers
 * migrate.
 */
export async function runWorkshop(args: WorkshopArgs): Promise<void> {
  try {
    const namer = await runNamerPhase({ brief: args.brief, getLlm: args.getLlm, emit: args.emit });
    const chosenName = namer.top_pick;
    const brandScoutOutput = await runBrandScoutPhase({
      brief: args.brief,
      chosenName,
      getLlm: args.getLlm,
      getSearch: args.getSearch,
      emit: args.emit,
    });
    await runFinishPhase({
      ...
      getLlm: args.getLlm,
      getSearch: args.getSearch,
      getImage: args.getImage,
      emit: args.emit,
    });
  } catch (err) {
    // Phase functions already emit workshop_error; we just suppress the throw
