...
import { runSuggestSimilarPhase } from "@/lib/orchestrator/workshop";
...
export async function POST(req: NextRequest) {
  ...
  const rejectedName = (body.rejectedName ?? "").trim();
  if (!rejectedName || rejectedName.length > 60 || /\n/.test(rejectedName)) {
    return new Response("rejectedName must be 1-60 characters, no newlines.", { status: 400 });
  }

  const scorecardResult = ScorecardSchema.safeParse(body.scorecard);
  if (!scorecardResult.success) {
    return new Response("Invalid scorecard.", { status: 400 });
  }

  const findingsInput = Array.isArray(body.findings) ? body.findings.slice(0, 10) : [];
  const findings: { category: string; finding: string }[] = [];
  for (const item of findingsInput) {
    const parsed = FindingSchema.safeParse(item);
    if (!parsed.success) {
      return new Response("Invalid findings.", { status: 400 });
    }
    ...
    async start(controller) {
      ...
      try {
        await runSuggestSimilarPhase({
          brief,
          rejectedName,
          rejectedScorecard: scorecardResult.data,
          rejectedFindings: findings,
          avoid,
