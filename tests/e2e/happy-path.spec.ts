...
const FAKE_KIT: BrandKit = {
  ...
  namesConsidered: [
    ...
    { name: "Mosaic", reasoning: "x", rejected: true, reason: "not top pick" },
  ],
  logos: [
...
test("happy path: each pane transitions and brand kit renders", async ({ page }) => {
  ...
  await expect(page.getByTestId("namer-pane")).toHaveAttribute("data-state", "done");
  await expect(page.getByTestId("brand-scout-pane")).toHaveAttribute("data-state", "done");
  await expect(page.getByTestId("designer-pane")).toHaveAttribute("data-state", "done");
